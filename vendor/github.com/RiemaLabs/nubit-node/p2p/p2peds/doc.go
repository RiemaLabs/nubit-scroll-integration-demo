...
//
//   - "{networkID}/p2p/eds/v0.0.1" where networkID is the network ID of the network. (e.g. "nubit")
//
...
// # Usage
//
// To use a p2peds client to request extended data squares from a peer, you must
// first create a new `p2peds.Client` instance by:
//
//	client, err := p2peds.NewClient(params, host)
//
// where `params` is a `p2peds.Parameters` instance and `host` is a `libp2p.Host` instance.
//
...
//
// To use a p2peds server to respond to requests for extended data squares from peers
// you must first create a new `p2peds.Server` instance by:
//
//	server, err := p2peds.NewServer(params, host, store)
//

...
func New(h hash.Hash, setters ...Option) *NamespacedMerkleTree {
	...
	hasher := NewNmtHasher(h, opts.NamespaceIDSize)
	opts.Hasher = hasher
	...
	return &NamespacedMerkleTree{
		ignoreMaxNs:      opts.IgnoreMaxNamespace,
		precomputedMaxNs: bytes.Repeat([]byte{0xFF}, int(opts.NamespaceIDSize)),
...
// func (n *NamespacedMerkleTree) commitKzg() ([]byte, error) {
// 	data := gokzg4844.Blob{}
// 	for i, h := range n.leafHashes {
// 		var r fr.Element
// 		b := r.SetBytes(h).Bytes()
// 		copy(data[i*32:], b[:])
// 	}
// 	commit, err := n.context.BlobToKZGCommitment(&data, 0)
// 	if err != nil {
// 		return nil, err
// 	}
// 	nLen := int(n.NamespaceSize())
// 	res := make([]byte, nLen*2+len(commit))
// 	copy(res[:nLen], n.minNID)
// 	copy(res[nLen:], n.maxNID)
// 	copy(res[nLen*2:], commit[:])

// 	return res, err
// }

// func (n *NamespacedMerkleTree) openKzg(index int) (*KzgOpen, error) {
// 	blob := gokzg4844.Blob{}
// 	for i, h := range n.leafHashes {
// 		copy(blob[i*32:], h[:])
// 	}

// 	evaluationPoint, err := n.context.DomainByIndex(index)
// 	if err != nil {
// 		return nil, err
// 	}

// 	intput := evaluationPoint.Bytes()
// 	inputPoint := gokzg4844.Scalar{}
// 	copy(inputPoint[:], intput[:])
// 	proof, _, err := n.context.ComputeKZGProof(&blob, inputPoint, 0)
// 	if err != nil {
// 		return nil, err
// 	}
// 	return &KzgOpen{
// 		index: index,
// 		value: n.leaves[index],
// 		proof: proof[:],
// 	}, nil
// }

// Proofs above this line are of KZG
// Proofs below this line are of the application layer
...
func (n *NamespacedMerkleTree) Prove(index int) (KzgOpen, error) {
	if index > n.Size() || index < 0 {
		return KzgOpen{}, ErrInvalidRange
	}

	proof, _, err := n.treeHasher.Open(n.leafHashes, index)
	if err != nil {
		return KzgOpen{}, err
	}
...
func (n *NamespacedMerkleTree) ProveRange(start, end int) (*NamespaceRangeProof, error) {
	...
	proof, err := BuildRangeProof(start, end, n.leaves, n.leafHashes, n.treeHasher)
	if err != nil {
		return &NamespaceRangeProof{}, err
	}
	return proof, nil
}
...
func (n *NamespacedMerkleTree) ProveNamespace(nID namespace.ID) (*NamespaceRangeProof, error) {
	...
	if !found {
		postIndex := n.calculateAbsenceIndex(nID)
		preIndex := postIndex - 1
		if err := n.validateRange(preIndex, postIndex); err != nil {
			return nil, err
		}
		return BuildAbsansProof(preIndex, postIndex, n.leaves, n.leafHashes, n.treeHasher)
	}
	...
	proof, err := BuildRangeProof(proofStart, proofEnd, n.leaves, n.leafHashes, n.treeHasher)
	if err != nil {
		return nil, err
	}
...
func BuildAbsansProof(preIndex, postIndex int, leaves, leafHashes [][]byte, hasher Hasher) (*NamespaceRangeProof, error) {
	res := &NamespaceRangeProof{
		InclusionOrAbsence: false,
	}

	preProof, _, err := hasher.Open(leafHashes, preIndex)
	if err != nil {
		return res, err
	}
	res.preIndex = preIndex
	res.openPreIndex = KzgOpen{preIndex, leaves[preIndex], preProof}
...
func BuildRangeProof(proofStart, proofEnd int, leaves, leafHashes [][]byte, hasher Hasher) (*NamespaceRangeProof, error) {
	res := &NamespaceRangeProof{
		InclusionOrAbsence: true,
	}

	size := len(leafHashes)
	startProof, _, err := hasher.Open(leafHashes, proofStart)
	if err != nil {
		return res, err
	}
	res.start = proofStart
	res.openStart = KzgOpen{
		index: proofStart,
		value: leaves[proofStart],
		proof: startProof,
	}

	proofEnd = proofEnd - 1
	endProof, _, err := hasher.Open(leafHashes, proofEnd)
	if err != nil {
		return res, err
	}
	res.end = proofEnd
	res.openEnd = KzgOpen{
		index: proofEnd,
		value: leaves[proofEnd],
		proof: endProof,
	}

	if proofStart > 0 {
		preIndex := proofStart - 1
		preProof, _, err := hasher.Open(leafHashes, preIndex)
		if err != nil {
			return res, err
		}
...
func (n *NamespacedMerkleTree) GetWithProof(nID namespace.ID) ([][]byte, *NamespaceRangeProof, error) {
	data := n.Get(nID)
...
func (n *NamespacedMerkleTree) Push(namespacedData namespace.PrefixedData) error {
	...
	if len(n.leaves) > MaxDataLengh {
		return errors.New("too many leaves")
	}
...
func (n *NamespacedMerkleTree) computeRoot() ([]byte, error) {
	if n.Size() == 0 {
		return n.treeHasher.EmptyRoot(), nil
	}
	// compute the root
	com, err := n.treeHasher.Commit(n.leafHashes)
	if err != nil {
		return nil, err
	}

	// all leaves have the same namespace = n.precomputedMaxNs
	if bytes.Compare(n.minNID, n.maxNID) > 0 {
		n.maxNID = n.minNID
	}

	res := make([]byte, int(n.treeHasher.NamespaceSize())*2)
	copy(res[:n.NamespaceSize()], n.minNID)
	copy(res[n.NamespaceSize():2*n.NamespaceSize()], n.maxNID)
	res = append(res, com...)
	n.visit(res, n.leafHashes...)
...
func (n *NamespacedMerkleTree) updateMinMaxID(id namespace.ID) {
	...
	if n.ignoreMaxNs {
		if !bytes.Equal(n.precomputedMaxNs, id) {
			if n.maxNID.Less(id) {
				n.maxNID = id
			}
			return

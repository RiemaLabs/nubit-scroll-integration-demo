...
//
// where networkID is the network ID of the nubit-node that is running the protocol. (e.g. "nubit")
//
// # Usage
//
// To use this protocol, you must first create a new `p2psub.PubSub` instance by:
//
//	pubsub, err := p2psub.NewPubSub(ctx, host, networkID)
//
// where host is the libp2p host that is running the protocol, and networkID is the network ID of the nubit-node
...
//
// Once you have started the `p2psub.PubSub` instance, you can broadcast a share by:
//
//	err := pubsub.Broadcast(ctx, notification)
//
// where `notification` is of type [p2psub.Notification].
//
...
//
// where `validator` is of type [p2psub.ValidatorFn] and `Notification` is the same as above.
//

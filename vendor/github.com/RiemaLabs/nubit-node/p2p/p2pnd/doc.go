...
//
//   - "{networkID}/p2p/nd/0.0.1" where networkID is the network ID of the network. (e.g. "nubit")
//
// The protocol uses protobuf to serialize and deserialize messages.
//
// # Usage
//
// To use a p2pnd client to request data from a peer, you must first create a new `p2pnd.Client` instance by:
//
// 1. Create a new client using `NewClient` and pass in the parameters of the protocol and the host:
//
//	client, err := p2pnd.NewClient(params, host)
//
...
//
// To use a p2pnd server to respond to requests from peers, you must first create a new `p2pnd.Server` instance by:
//
...
//
//	server, err := p2pnd.NewServer(params, host, store, storeShareGetter)
//
